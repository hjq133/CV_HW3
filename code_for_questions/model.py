    ...
    def train_nn(self):
        ...
        for epoch in range(hp.num_epochs):
            ...
            for index in range(len(indices)):
                ...
                softmax_grad = np.copy(pred.reshape(1, self.num_classes))
                softmax_grad[:, gt_label] -= 1
                delta_W = np.dot(img.reshape(-1, 1), softmax_grad)
                delta_b = softmax_grad

                # Step 5:
                # Update self.W and self.b using the gradient terms and the self.learning_rate hyperparameter.
